...
int TCPSocket::readFromSocket (char * buffer, int maxBytes ) { // Blocking read data operation from socket
    if ( buffer != NULL ) {
            ...
            int read = 0;
            read = recv (sock,buffer,maxBytes,0); // invoke recv system call
...
int TCPSocket::writeToSocket (const char * buffer, long maxBytes ) //Write data to socket
{
    if ( buffer != NULL ) { // If buffer is not NULL
        int wrote = 0;
        wrote = send (sock,buffer,maxBytes,0); // user send system call to write data to socket
...
{
    // printf ("Destructing TCPSocket::~TCPSocket()\n");
    shutDown(); // Shutdown
