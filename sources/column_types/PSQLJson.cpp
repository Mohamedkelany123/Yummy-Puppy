...
{
    return  "\t\tvoid "+class_name+"::set_"+column_name+"( json _value, bool set_null) {if (seeder_readonly) return; update_flag.set("+std::to_string(col_index)+"); "+field_name+"=_value; if(set_null) null_flag.set("+std::to_string(col_index)+"); } \n"+
        "\t\tvoid "+class_name+"::set_"+column_name+"( string _value, bool set_null) { if (seeder_readonly) return; update_flag.set("+std::to_string(col_index)+"); "+field_name+"=json::parse(_value); if(set_null) null_flag.set("+std::to_string(col_index)+"); } \n";
}
